 ...
 * Uses HTML <span> instead of SVG <text> — SVG font loading is           *
 * unreliable across browsers/print/PDF contexts. HTML spans with inline   *
 * fontFamily always resolve from the document font stack.                 *
 * ────────────────────────────────────────────────────────────────────── */
function ChannelLogo({ channel, isActive = true }: { channel: string; isActive?: boolean }) {
  const inactive: React.CSSProperties = !isActive ? { filter: 'grayscale(100%)', opacity: 0.4 } : {};
  const word: React.CSSProperties = {
    fontFamily: 'Arial, Helvetica, sans-serif', fontWeight: 600,
    fontSize: 5.5, letterSpacing: '1.5px', color: 'rgba(255,255,255,0.80)',
    marginTop: 2, textTransform: 'uppercase', lineHeight: 1,
  };
  ...
  if (channel === 'Booking.com') return (
    <div className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0 shadow-md overflow-hidden select-none"
      style={{ background: 'linear-gradient(145deg,#003580,#002060)', ...inactive }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', lineHeight: 1 }}>
        <span style={{ ...initial, fontSize: 23, color: '#fff' }}>B</span>
        <span style={{ ...initial, fontSize: 23, color: '#7AB3FF' }}>.</span>
      </div>
      <span style={word}>BOOKING</span>
    </div>
  );
  ...
  if (channel === 'Airbnb') return (
    <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-md overflow-hidden"
      ...
      {/* Bélo icon — SVG path only, no text, always renders correctly */}
  ...
  if (channel === 'Gathern') return (
    <div className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0 shadow-md overflow-hidden select-none"
      ...
      <span style={{ ...initial, fontSize: 22, color: '#fff' }}>G</span>
      <span style={word}>GATHERN</span>
    </div>
  );
  ...
  if (channel === 'Agoda') return (
    <div className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0 shadow-md overflow-hidden select-none"
      ...
      <span style={{ ...initial, fontSize: 22, color: '#fff' }}>A</span>
      <span style={word}>AGODA</span>
    </div>
  );
  ...
  if (channel === 'Expedia') return (
    <div className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0 shadow-md overflow-hidden select-none"
      ...
      <span style={{ ...initial, fontSize: 22, color: '#FFC72C' }}>E</span>
      <span style={{ ...word, letterSpacing: '1.2px' }}>EXPEDIA</span>
    </div>
  );

  return (
    <div className="w-12 h-12 rounded-2xl bg-slate-200 flex items-center justify-center flex-shrink-0 select-none"
      style={inactive}>
