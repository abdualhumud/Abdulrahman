...
function ChannelLogo({ channel, isActive = true }: { channel: string; isActive?: boolean }) {
  ...
  if (channel === 'Waffy') return (
    ...
    >
      {/* Waffy: Arabic "وفّي" / "W" wordmark + "WAFFY" subtext */}
      <svg width="44" height="38" viewBox="0 0 44 38" fill="none">
...
/* ── Waffy Escrow Integration Panel ──────────────────────────────────────── */
function WafiIntegrationPanel() {
  const { t, lang } = useLang();
  const tc = t.channels;
  const [escrowLog, setEscrowLog] = useState<Array<{ id: number; time: string; text: string; color: string }>>([]);
  const [simulating, setSimulating] = useState(false);
  const logRef = useRef<HTMLDivElement>(null);
  let _wid = 0;
  ...
  const addLog = (text: string, color = 'text-slate-400') => {
    const now = new Date();
    const time = `${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}:${String(now.getSeconds()).padStart(2,'0')}`;
    setEscrowLog(prev => [...prev.slice(-29), { id: ++_wid, time, text, color }]);
  };
  ...
  const ESCROW_STEPS = [
    {
      num: '1', color: 'bg-emerald-700',
      text: lang === 'ar'
        ? 'POST /transactions — إنشاء معاملة ضمان جديدة للضيف'
        : 'POST /transactions — create escrow hold for guest deposit',
    },
    {
      num: '2', color: 'bg-blue-600',
      text: lang === 'ar'
        ? 'مشاركة رابط الدفع الآمن مع الضيف عبر WhatsApp / SMS'
        : 'Share secure payment link with guest via WhatsApp / SMS',
    },
    {
      num: '3', color: 'bg-violet-600',
      text: lang === 'ar'
        ? 'Webhook: transaction.funded — المبلغ محتجز في الضمان حتى فحص المغادرة'
        : 'Webhook: transaction.funded — funds held until check-out inspection',
    },
    {
      num: '4', color: 'bg-teal-600',
      text: lang === 'ar'
        ? 'POST /release أو /refund — بعد اكتمال INSPECTION_DONE في صفحة التنظيف'
        : 'POST /release or /refund — triggered after INSPECTION_DONE in Cleaning page',
    },
  ];

  const WEBHOOK_EVENTS: Array<{ event: string; desc: string; color: string }> = [
  ...
  return (
    <div className="card p-6 space-y-5">
      ...
      <div>
        ...
        <p className="text-[10px] text-slate-400 mt-2 ps-2 border-s-2 border-emerald-200">
          {tc.wafiEscrowDesc}
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-[10px] text-slate-400 mt-2 ps-2 border-s-2 border-violet-200">
          {tc.wafiWebhookDesc}
        </p>
      </div>

      {/* Escrow simulation log */}
      <div className="rounded-2xl overflow-hidden border border-slate-200">
        <div className="flex items-center justify-between bg-slate-800 px-4 py-2">
          <div className="flex items-center gap-2">
        ...
        >
          {escrowLog.length === 0 && (
            <p className="text-slate-600 italic">Press &quot;Simulate Escrow&quot; to see the full escrow lifecycle.</p>
          )}
          {escrowLog.map(e => (
            <div key={e.id} className="flex gap-2">
              <span className="text-slate-600 flex-shrink-0">[{e.time}]</span>
...
export default function ChannelsPage() {
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      {/* Waffy Escrow Integration — Escrow payments + webhook events */}
      <WafiIntegrationPanel />
